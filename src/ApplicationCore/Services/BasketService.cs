...
namespace Sula.Shipment.ApplicationCore.Services
{
    public class BasketService : IBasketService
    {
        ...
        public async Task SetCheckoutProperties(int basketId, string checkoutMethod, string courierArrivalTime, string recipientName, string recipientSurname, string recipientAddress, string recipientPhoneNo)
        {
            var basketSpec = new BasketWithItemsSpecification(basketId);
            var basket = await _basketRepository.FirstOrDefaultAsync(basketSpec);
            Guard.Against.NullBasket(basketId, basket);

            basket.SetNewCheckoutMethod(checkoutMethod);
            basket.SetNewCourierArrivalTime(courierArrivalTime);
            basket.SetNewRecipientName(recipientName);
            basket.SetNewRecipientSurname(recipientSurname);
            basket.SetNewRecipientAddress(recipientAddress);
            basket.SetNewRecipientPhoneNo(recipientPhoneNo);
